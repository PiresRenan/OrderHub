...
fixture effects, including 100 units on hand and no Orders. User, Staff and
Tenant UUIDs are allocated anew; retrieve current selectors from `/fixture`.
Product, Variant and Customer fixture UUIDs and synthetic subjects are stable.
Do not depend on data surviving a restart.
...
`POST /tokens/{persona}`. Credentials expire after five minutes. Only the four
listed personas are admitted. There is no arbitrary-subject or role claim input,
...
| `staff` | Initial Tenant governance Staff established through the first-Staff proof ceremony | Catalog and Inventory administration |
| `customer` | Internal User linked by the Customer owner proof flow | Create and view its Customer Orders |
| `outsider` | Bound internal User without fixture Tenant membership | Exercise denied Tenant access |
...
The seed uses real Users, Authorization, Tenants, Workforce, Catalog, Inventory
and Customer application services. The initial Platform grants are the explicit
test-only trust-root ceremony. A single fixture-only insert establishes the
synthetic CustomerProfile because no Customer creation application command
