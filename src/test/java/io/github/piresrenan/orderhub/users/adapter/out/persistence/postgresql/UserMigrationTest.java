...
class UserMigrationTest {
  ...
  void migratesEmptyDatabaseIntoUsersSchema() {
    ...
    // history including the external identity binding migration.
    // Covers: cumulative V1 through V5 execution plus Users-owned tables required
    // for internal identity, Tenant membership and external identity binding.
    // Prevents: Users persistence depending on manual database preparation,
    // modification of accepted migrations or omission of external identity storage.
    ...
    var externalIdentityBindingsTableExists = jdbcTemplate.queryForObject("""
        SELECT EXISTS (
            SELECT 1
            FROM information_schema.tables
            WHERE table_schema = 'users'
    ...
    assertThat(externalIdentityBindingsTableExists)
        .isTrue();
  }
}
